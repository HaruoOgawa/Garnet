...
namespace graphics
{
	class CPostProcessToneMapping : public IPostProcessFilter, public scriptable::CValueRegistry
	{
		std::string m_TargetPassName;

		std::shared_ptr<graphics::CFrameRenderer> m_FrameRenderer;
		std::shared_ptr<graphics::CFrameRenderer> m_ResultFrameRenderer;

		std::shared_ptr<graphics::CTexture> m_TargetTexture;
	public:
		CPostProcessToneMapping(const std::string& TargetPassName);
		virtual ~CPostProcessToneMapping();
