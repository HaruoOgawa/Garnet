...
// "transfer of control bypasses initialization of"の対策でgoto文からプリプロセッサ呼び出しに変更している
// ToDo: 何かあったら戻す
// https://stackoverflow.com/questions/5136295/switch-transfer-of-control-bypasses-initialization-of-when-calling-a-function
#define DoErrorExit { if (hOutputReadTmp) { CloseHandle(hOutputReadTmp); } if (hOutputWrite){ CloseHandle(hOutputWrite); } if (hErrorWrite){ CloseHandle(hErrorWrite); } return;}
