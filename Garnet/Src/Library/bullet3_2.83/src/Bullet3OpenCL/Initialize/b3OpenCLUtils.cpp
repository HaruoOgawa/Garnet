...
cl_program b3OpenCLUtils_compileCLProgramFromString(cl_context clContext, cl_device_id device, const char* kernelSourceOrg, cl_int* pErrNum, const char* additionalMacrosArg , const char* clFileNameForCaching, bool disableBinaryCaching)
{
	...
	char* bla=0;
		...
		CreateDirectory((LPCWCHAR)sCachedBinaryPath,0);
		{

			HANDLE binaryFileHandle = CreateFile((LPCWCHAR)binaryFileName,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
			...
			if (binaryFileValid)
			{
				HANDLE srcFileHandle = CreateFile((LPCWCHAR)clFileNameForCaching,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
				...
				if (srcFileHandle==INVALID_HANDLE_VALUE)
				{
					...
					for (int i=0;(srcFileHandle==INVALID_HANDLE_VALUE) && i<5;i++)
					{
						...
						srcFileHandle = CreateFile((LPCWCHAR)relativeFileName,GENERIC_READ,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
					}
