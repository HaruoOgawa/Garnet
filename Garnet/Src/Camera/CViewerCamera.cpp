...
namespace camera
{
	...
	void CViewerCamera::MouseAction(float DeltaTime, const std::shared_ptr<input::CInputState>& InputState)
	{
		...
		{
			float RotperSecond = 60.0f * InputState->GetGloabalInputSpeed(); // 1秒あたり何度回転させるか
		...
		{
			float MoveRate = 60.0f * InputState->GetGloabalInputSpeed(); // 1秒あたりどれぐらい移動するか
		...
		else if (InputState->IsMouseWheeled())
		{
			...
			float MoveRate = 30.0f * InputState->GetGloabalInputSpeed(); // 1秒あたりどれぐらい移動するか
	...
	void CViewerCamera::KeyAction(float DeltaTime, const std::shared_ptr<input::CInputState>& InputState)
	{
		...
		if (KeyInputMap.size() > 0)
		{
			for (const auto& KeyInput : KeyInputMap)
			{
				...
				//
				float MoveRate = 3.0f * InputState->GetGloabalInputSpeed(); // 1秒あたりどれぐらい移動するか
