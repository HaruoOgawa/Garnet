...
namespace gui
{
	...
	bool CGUIResourcesTab::Draw()
	{
		...
		{
			if (!DrawDirGUI(m_RootDir)) return false;
	...
	bool CGUIResourcesTab::DrawDirGUI(const std::shared_ptr<SDirectoryInfo>& Dir)
	{
		// Emscriptenはサポートしない
#ifdef __EMSCRIPTEN__
		return true;
		...
		if (IsOpened)
		{
			...
			{
				if (!DrawDirGUI(SubDir)) return false;
			}
