...
namespace resource
{
	...
	bool C3DObjectLoader::Import(api::IGraphicsAPI* pGraphicsAPI, physics::IPhysicsEngine* pPhysicsEngine)
	{
		...
		if (Extention == "gltf")
		{
			...
			if (!gltf::CGLTFImporter::ImportFromString(pGraphicsAPI, m_File->GetData(), BaseDir, m_TargetObject.get(), m_BaseMaterialFrame, this, m_TargetRigType)) return false;
#endif
		}
		else if (Extention == "glb")
		{
#ifdef USE_GLTF
			if (!gltf::CGLTFImporter::ImportFromMemory(pGraphicsAPI, m_File->GetData(), m_TargetObject.get(), m_BaseMaterialFrame, this, m_TargetRigType)) return false;
#endif
		}
...
#if defined(USE_FBX)
#ifdef USE_SMALL_FBX
			if (!fbx::CSmallFBXImporter::ImportFBX(pGraphicsAPI, m_File->GetData(), m_TargetObject.get(), m_BaseMaterialFrame, this, m_TargetRigType)) return false;
#else
			if (!fbx::CFBXImporter::ImportFBX(pGraphicsAPI, m_FileName, m_TargetObject.get(), m_BaseMaterialFrame, this, m_TargetRigType)) return false;
#endif // USE_SMALL_FBX
#endif
		}
		...
		{
#ifdef USE_MMD
			if (!mmd::CPmxImporter::ImportPmx(pGraphicsAPI, pPhysicsEngine, m_FileName, m_File->GetData(), m_TargetObject.get(), m_BaseMaterialFrame, this, m_TargetRigType)) return false;
#endif
		}
		else
		{
