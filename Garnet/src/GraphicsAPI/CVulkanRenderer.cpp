...
namespace renderer
{
	...
	bool CVulkanRenderer::CreateGraphicsPipeline(const CRendererCreateInfo& createInfo)
	{
		...
		auto attributeDescriptions = GetAttributeDestriptions();
	...
	VkVertexInputBindingDescription CVulkanRenderer::GetBindingDescription() {
	...
	std::array<VkVertexInputAttributeDescription, 3> CVulkanRenderer::GetAttributeDestriptions() {
		// アトリビュート(頂点データ)の設定
