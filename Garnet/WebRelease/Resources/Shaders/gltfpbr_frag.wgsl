...
fn main_1() {
    ...
    let _e251 = reflectColor;
    let _e252 = (((_e244.xyz * _e242) * (_e247 + _e248)) + _e251);
    col[0u] = _e252.x;
    col[1u] = _e252.y;
    col[2u] = _e252.z;
    let _e260 = ubo.useOcclusionTexture;
    if (_e260 != 0) {
        let _e262 = f_Texcoord_1;
        let _e263 = textureSample(occlusionTexture, occlusionTextureSampler, _e262);
        ao = _e263.x;
        let _e265 = col;
        let _e267 = col;
        let _e269 = ao;
        let _e272 = ubo.occlusionStrength;
        let _e274 = mix(_e265.xyz, (_e267.xyz * _e269), vec3<f32>(_e272));
        col[0u] = _e274.x;
        col[1u] = _e274.y;
        col[2u] = _e274.z;
    }
    let _e282 = ubo.useEmissiveTexture;
    if (_e282 != 0) {
        let _e284 = f_Texcoord_1;
        let _e285 = textureSample(emissiveTexture, emissiveTextureSampler, _e284);
        param_8 = _e285;
        let _e286 = SRGBtoLINEARvf4_((&param_8));
        let _e289 = ubo.emissiveFactor;
        emissive = (_e286.xyz * _e289.xyz);
        let _e292 = emissive;
        let _e293 = col;
        let _e295 = (_e293.xyz + _e292);
        col[0u] = _e295.x;
        col[1u] = _e295.y;
        col[2u] = _e295.z;
    }
    let _e302 = col;
    let _e304 = pow(_e302.xyz, vec3<f32>(0.4545454680919647, 0.4545454680919647, 0.4545454680919647));
    col[0u] = _e304.x;
    col[1u] = _e304.y;
    col[2u] = _e304.z;
    let _e312 = baseColor[3u];
    col[3u] = _e312;
    let _e314 = col;
    outColor = _e314;
    return;
